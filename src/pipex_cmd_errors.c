...
/*   Updated: 2024/06/20 13:48:48 by asohrabi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_env_pack	full_cmd_check(t_cmd *cmd_execution, t_env_pack env_pack)
{
		...
		&& is_builtin(cmd_execution) == -1 && !cmd_execution->file_error)
	{
		if (cmd_execution->dir)
			env_pack = cmd_dir(cmd_execution, env_pack);
		else
		{
			if (cmd_execution->exist)
				env_pack = cmd_permission(cmd_execution, env_pack);
			else
				env_pack = cmd_not_found(cmd_execution, env_pack);
		}
	}
	return (env_pack);
}
